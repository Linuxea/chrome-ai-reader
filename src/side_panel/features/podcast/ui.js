...
let _cardHandlers = null;
let _statusHandlers = null;

export function initUICallbacks({ cardHandlers, statusHandlers }) {
  _cardHandlers = cardHandlers;
...
function createPodcastCard(quotePreview, chatArea) {
  const existing = chatArea.querySelector('.podcast-card');
  ...
  progressBar.addEventListener('mousedown', (e) => {
    ...
    _cardHandlers.onSeekMouse(e, card, progressBar);
    const onMove = (ev) => { if (isDragging) _cardHandlers.onSeekMouse(ev, card, progressBar); };
    const onUp = (ev) => {
      if (!isDragging) return;
      isDragging = false;
      progressBar.classList.remove('dragging');
      _cardHandlers.onSeekMouse(ev, card, progressBar);
  ...
  progressBar.addEventListener('touchstart', (e) => {
    ...
    isDragging = true;
    progressBar.classList.add('dragging');
    _cardHandlers.onSeekTouch(e, card, progressBar);
  }, { passive: false });
  progressBar.addEventListener('touchmove', (e) => {
    if (isDragging) { e.preventDefault(); _cardHandlers.onSeekTouch(e, card, progressBar); }
  }, { passive: false });
...
function updateTranscriptHighlight(roundIdx, currentCard) {
...
function updateCardStatus(card, status, text) {
  ...
  switch (status) {
    ...
    case 'playing':
      ...
      _statusHandlers.addDownloadButton(card);
      break;
    case 'done':
    ...
    case 'error':
      ...
      card.querySelector('.podcast-retry-btn').addEventListener('click', () => {
        _statusHandlers.cleanupPodcast();
        card.remove();
        _statusHandlers.handlePodcastClick();
      });
      break;
  }
}

function restoreWelcomeIfNeeded(chatArea) {
