const TRANSLATIONS = {
  zh: {
    ...
    'settings.llm.apiKey.hint': 'API Key 保存在浏览器的扩展存储中（开启 Chrome 同步时会随账号同步到其他设备），只会发送给你配置的 API 地址。',
  ...
  en: {
    ...
    'settings.llm.apiKey': 'API Key',
    'settings.llm.apiKey.ph': 'sk-...',
    'settings.llm.apiKey.hint': 'Your API Key is kept in the extension\'s browser storage (synced to your other devices when Chrome Sync is on) and is only sent to the API address you configure.',
