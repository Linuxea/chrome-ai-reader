const TRANSLATIONS = {
  zh: {
    ...
    'settings.llm.provider.openai': 'OpenAI 兼容（DeepSeek / OpenAI / Ollama / 火山方舟 …）',
    'settings.llm.provider.anthropic': 'Anthropic（Claude 原生接口）',
    'settings.llm.fastModel': '快速模型（可选）',
    'settings.llm.fastModel.ph': '留空则全部使用上面的模型',
    'settings.llm.fastModel.hint': '推荐问题、标题、深度批阅、沉浸式翻译、测验等轻量任务使用这个更便宜更快的模型。',
    'settings.llm.citations': '回答附带原文引用（点击 [#N] 跳转到网页对应段落）',
    'settings.llm.agentMode': '智能体模式（模型可自行搜索页面、查看选区、查找相关阅读，实验）',
    'settings.llm.apiKey.hint': 'API Key 只保存在本机的扩展存储中（不随 Chrome 同步），只会发送给你配置的 API 地址。',
    ...
    'settings.llm.model.hint': '选择或输入要使用的模型名称。点击刷新按钮从当前 API 地址获取可用模型列表。上传和截图的图片会直接发给模型，请使用支持图片输入的多模态模型。',
    ...
    'settings.podcast.direct': '播客直连（无需本地代理）',
    'settings.podcast.direct.hint': '开启后由扩展直接连接火山引擎播客接口（通过 declarativeNetRequest 注入鉴权请求头），不用再运行 proxy/。直连失败时会自动回退到本地代理。',
    ...
    'settings.usage': '用量统计',
    'settings.usage.hint': '按模型统计的请求次数和 token 用量（只统计接口返回了用量数据的请求）。',
    'settings.usage.today': '今天',
    'settings.usage.week': '最近 7 天',
    'settings.usage.month': '最近 30 天',
    'settings.usage.clear': '清空统计',
    'settings.usage.clearConfirm': '确定清空用量统计吗？（不会影响对话记录）',
    'settings.usage.empty': '这段时间还没有用量记录。',
    'settings.usage.model': '模型',
    'settings.usage.requests': '请求',
    'settings.usage.input': '输入 tokens',
    'settings.usage.output': '输出 tokens',
    ...
    'settings.data.includeSecrets': '导出时包含 API Key 等密钥（导出文件为明文，请妥善保管）',
    ...
    'sidebar.visionCapture': '截取当前画面',
    'sidebar.visionCaptureFull': '整页截屏',
    'sidebar.remove': '移除',
    'sidebar.input.ph': '输入问题...',
    'sidebar.send': '发送',
    'sidebar.clearQuote': '清除引用',
    'sidebar.welcome': '打开任意网页，点击上方按钮或输入问题开始使用。',
    'onboarding.needConfig': '开始之前，先完成配置',
    'onboarding.hint': '配置 API Key 与模型名称后，即可针对当前网页提问、总结、生成播客',
    'history.confirmDelete': '再次点击确认删除',
    ...
    'action.edit': '编辑',
    'action.edit.save': '保存并发送',
    'action.edit.cancel': '取消',
    'action.copy': '复制',
    'action.copied': '已复制',
    ...
    'screenshot.defaultName': '截图 {time}',
    'screenshot.fullPageName': '整页截图 {n}/{total} {time}',
    'screenshot.fullPageProgress': '正在截屏 {done}/{total}…',
    'error.visionPayloadTooLarge': '图片过多可能发送失败，建议分多轮对话',
    'error.imageLostAfterReload': '图片已在重启后丢失',
    'error.extractFailed': '页面内容提取失败',
    'error.pdfFetch': 'PDF 下载失败（{status}）',
    'error.pdfFileAccess': '无法读取本地 PDF：请在扩展详情页打开「允许访问文件网址」',
    'error.pdfTooLarge': 'PDF 太大，暂不支持（上限 80 MB）',
    'error.pdfNoText': '这个 PDF 没有文字层（可能是扫描件），暂时无法读取',
    'error.pdfParse': 'PDF 解析失败：{message}',
    'error.pageUnsupported': '无法读取这个页面：浏览器内置页面（如设置页、扩展商店、新标签页）不支持，请在普通网页上使用',
    ...
    'action.quiz': '测验',
    'quiz.title': '📝 阅读测验',
    'quiz.generating': '正在根据文章出题…',
    'quiz.failed': '生成测验失败',
    'quiz.parseFailed': '模型返回的题目格式无法解析，请重试',
    'quiz.score': '得分：{correct} / {total}',
    'quiz.flashcards': '记忆卡片（点击翻面）',
    'quiz.flip': '点击翻面',
    'quiz.exportAnki': '导出为 Anki 卡片（CSV）',
    'quiz.exported': '已导出，可在 Anki 中"导入文件"',
    'branch.previous': '上一个版本',
    'branch.next': '下一个版本',
    'action.immersive': '对照翻译',
    'immersive.started': '正在网页中逐段插入译文，再点一次可关闭',
    'immersive.failed': '部分段落翻译失败：{error}',
    'notes.title': '高亮与笔记',
    'notes.highlightSelection': '高亮选中文字并保存到笔记',
    'notes.exportAll': '导出全部笔记',
    'notes.exportHeading': '我的阅读笔记',
    'notes.empty': '这个页面还没有高亮。在网页上选中文字，点引用栏的 🖍 或右键"高亮"即可保存。',
    'notes.notePlaceholder': '写点笔记…',
    'notes.ask': '就此提问',
    'notes.delete': '删除',
    'notes.noSelection': '请先在网页上选中要高亮的文字',
    ...
    'agent.tool.get_selection': '查看选中文字',
    'agent.tool.find_related_pages': '查找相关页面',
    'agent.tool.search_reading_history': '搜索阅读历史',
    'agent.tool.list_open_tabs': '列出标签页',
    'agent.tool.read_tab': '读取标签页',
    'agent.tool.highlight_paragraph': '高亮段落',
    'citation.unavailable': '这条引用对应的页面内容已不可用（页面已跳转或重新打开）。',
    'citation.notFound': '在当前页面中没有找到这段原文。',
    'citation.quote': '引用原文：{text}',
    'ai.truncatedByLimit': '回答达到长度上限，已被截断。可以让我"继续"。',
    'ai.refusedPartway': '模型中途拒绝继续回答。',
    'ai.usage': '{model} · 输入 {in} / 输出 {out} tokens',
    'error.refused': '模型拒绝回答这个请求。',
    'error.streamTimeout': '模型长时间没有响应，已放弃本次请求。请稍后重试。',
    'ai.truncated': '[内容过长，已截断]',
    'markdown.blockedImage': '🖼 外部图片（点击打开）：{alt}',
  ...
  en: {
    ...
    'settings.llm.apiKey': 'API Key',
    'settings.llm.apiKey.ph': 'sk-...',
    'settings.llm.provider': 'API type',
    'settings.llm.provider.openai': 'OpenAI-compatible (DeepSeek / OpenAI / Ollama / Volcengine Ark …)',
    'settings.llm.provider.anthropic': 'Anthropic (native Claude API)',
    'settings.llm.fastModel': 'Fast model (optional)',
    'settings.llm.fastModel.ph': 'Leave empty to use the model above for everything',
    'settings.llm.fastModel.hint': 'Used for light work — suggested questions, titles, deep annotation, immersive translation, quizzes — to save cost and time.',
    'settings.llm.citations': 'Cite the page in answers (click [#N] to jump to that paragraph)',
    'settings.llm.agentMode': 'Agent mode (the model may search the page, read the selection and look up related reading; experimental)',
    'settings.llm.apiKey.hint': 'Your API Key is stored only on this device (not synced by Chrome) and is only sent to the API address you configure.',
    ...
    'settings.llm.model.hint': 'Select or enter the model name. Click refresh to fetch available models from the current API base. Uploaded and captured images are sent directly to the model, so use a multimodal model that accepts image input.',
    ...
    'settings.podcast.direct': 'Podcast direct mode (no local proxy)',
    'settings.podcast.direct.hint': 'Connects to the Volcengine podcast API straight from the extension (auth headers are added with declarativeNetRequest), so proxy/ no longer has to run. Falls back to the local proxy if the direct connection fails.',
    ...
    'settings.usage.hint': 'Requests and tokens per model (only requests whose API reported usage).',
    'settings.usage.today': 'Today',
    'settings.usage.week': 'Last 7 days',
    'settings.usage.month': 'Last 30 days',
    'settings.usage.clear': 'Clear statistics',
    'settings.usage.clearConfirm': 'Clear the usage statistics? (Conversations are not affected.)',
    'settings.usage.empty': 'No usage recorded in this period.',
    ...
    'settings.data.includeSecrets': 'Include API keys and other secrets in the export (the file is plain text — keep it safe)',
    ...
    'sidebar.visionCapture': 'Capture visible area',
    'sidebar.visionCaptureFull': 'Capture full page',
    'sidebar.remove': 'Remove',
    'sidebar.input.ph': 'Ask anything...',
    'sidebar.send': 'Send',
    ...
    'onboarding.needConfig': 'Finish setup to get started',
    'onboarding.hint': 'Configure your API Key and model to ask questions about the current page.',
    ...
    'screenshot.defaultName': 'Screenshot {time}',
    'screenshot.fullPageName': 'Full-page screenshot {n}/{total} {time}',
    'screenshot.fullPageProgress': 'Capturing {done}/{total}…',
    ...
    'error.pdfFetch': 'Could not download the PDF ({status})',
    'error.pdfFileAccess': 'Cannot read a local PDF: turn on "Allow access to file URLs" on the extension\'s details page',
    'error.pdfTooLarge': 'This PDF is too large (limit 80 MB)',
    'error.pdfNoText': 'This PDF has no text layer (probably a scan), so it cannot be read',
    'error.pdfParse': 'Failed to parse the PDF: {message}',
    'error.pageUnsupported': 'This page cannot be read: browser pages (settings, the Web Store, the new tab page) are not supported — please use it on a regular web page',
    ...
    'quiz.generating': 'Writing questions from the article…',
    'quiz.failed': 'Could not create the quiz',
    'quiz.parseFailed': 'The model\'s answer could not be read as a quiz — please try again',
    'quiz.score': 'Score: {correct} / {total}',
    'quiz.flashcards': 'Flashcards (click to flip)',
    'quiz.flip': 'Click to flip',
    'quiz.exportAnki': 'Export as Anki cards (CSV)',
    'quiz.exported': 'Exported — use "Import File" in Anki',
    'branch.previous': 'Previous version',
    'branch.next': 'Next version',
    'action.immersive': 'Bilingual',
    'immersive.started': 'Adding a translation under each paragraph — click again to turn it off',
    'immersive.failed': 'Some paragraphs could not be translated: {error}',
    ...
    'notes.exportHeading': 'My reading notes',
    'notes.empty': 'No highlights on this page yet. Select text on the page, then click 🖍 on the quote bar or right-click → Highlight.',
    'notes.notePlaceholder': 'Add a note…',
    'notes.ask': 'Ask about this',
    'notes.delete': 'Delete',
    'notes.noSelection': 'Select some text on the page first',
    ...
    'agent.tool.get_selection': 'Check selection',
    'agent.tool.find_related_pages': 'Find related pages',
    'agent.tool.search_reading_history': 'Search reading history',
    'agent.tool.list_open_tabs': 'List tabs',
    'agent.tool.read_tab': 'Read tab',
    'agent.tool.highlight_paragraph': 'Highlight paragraph',
    ...
    'citation.quote': 'Cited passage: {text}',
    ...
    'error.refused': 'The model declined this request.',
    'error.streamTimeout': 'The model stopped responding, so the request was abandoned. Please try again.',
    'ai.truncated': '[Content too long, truncated]',
    'markdown.blockedImage': '🖼 External image (click to open): {alt}',
